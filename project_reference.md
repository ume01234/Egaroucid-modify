...
**目標**: 駒数差を小さくして接戦にする。究極的な目標は、ユーザーがどう打っても引き分けで終わることであるが、これが難しい場合の実装としては駒数差が最小で勝利するなどとしても良い。

**方針**: 評価関数を「駒数差の絶対値を最小化」するように変更

**議論が必要な機能,UIの変更**: 本プロジェクトでは、Egaroucidの設計をなるべく引き継ぎつつ、制作するものは接待AIである。そのため、現在ある"AIの強さの調整"は不要かと思われる。この部分については本当に不要かどうか、消した場合にも設計に不整合が起こらないようにするためにはどうすればいいかを調査・議論する必要がある。
